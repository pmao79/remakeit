...
const ProcessStep: React.FC<{
  ...
  delay: number;
  id: string;
}> = ({ number, titleKey, descriptionKey, delay, id }) => {
  const { t } = useLanguage();
  
  return (
    <RevealSection delay={delay} className="relative">
      <div id={id} className="relative z-10 glass-panel rounded-lg p-8 h-full">
        <div className="absolute -top-8 -left-8 w-16 h-16 bg-brand-teal text-black rounded-full flex items-center justify-center text-2xl font-bold" aria-hidden="true">
          {number}
        </div>
        <div className="pt-4">
      ...
      {number < 4 && (
        <div className="hidden md:block absolute top-1/2 right-0 transform translate-x-1/2 -translate-y-1/2 z-0" aria-hidden="true">
...
const Process: React.FC = () => {
  ...
  return (
    <section id="process" className="py-20 relative overflow-hidden">
      <div className="absolute top-1/2 left-1/3 w-96 h-96 bg-brand-purple/20 blur-[150px] rounded-full pointer-events-none" aria-hidden="true"></div>
